  ...
  yolov8_msgs__msg__Yolov8Inference__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yolov8_msgs__msg__Yolov8Inference * data =
      (yolov8_msgs__msg__Yolov8Inference *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yolov8_msgs__msg__Yolov8Inference__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          yolov8_msgs__msg__Yolov8Inference__fini(&data[i]);
        }
        free(data);
        return false;
      }
